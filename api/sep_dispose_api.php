...
try {
    ...
    $stmt = $pdo->prepare(
        "UPDATE tbl_sep SET current_condition = 'Disposed' WHERE sep_id = ? AND current_condition = 'Unserviceable' AND assigned_to_user_id IS NULL"
    );
    $stmt->execute([$sep_id]);
    ...
    error_log("Dispose API Error: " . $e->getMessage());
